...
prob1c.fun <- function(a,b){
  result <- ifelse(a > b, b*factorial(a), b-a)
  return(result)
}
```
...
\color{blue}
Yes, those are the correct results.
...
\color{blue}
This second version with the 'ifelse()' command was much easier to write and
it worked much faster than using the for loop.
...
FizzBuzz <- function(n){
  aa <- 1:n
  result <- ifelse(aa %% 3 == 0 & aa %% 5 == 0, "Fizz Buzz",
                   ifelse(aa %% 3 == 0, "Fizz",
                          ifelse(aa %% 5 == 0, "Buzz", aa)))
  return(result)
}
```
